...
c(chalk.hex('#EEEEEE')(`▒▓▓████▓▓▒▒░░░░                                                               `));
c(chalk.hex('#EEEEEE')(`█████████▓▓▒▒░░                                                               `));
c(chalk.bold.green(`Build a site with json ${pjson.version}`));
showHelp();
c();
}
...
function showHelp() {
  c(chalk.yellow(`
Try these commands:`));
  c(chalk.magenta(`${appName} new my-epic-plox-site-2020`), chalk.blue(` - Creates a plox site`));
  c(chalk.magenta(`${appName} page page-of-amazing-content`), chalk.blue(` - Adds a page to your site`));
  c(chalk.magenta(`${appName} deploy`), chalk.blue(` - Send your site to the clouds`));
...
function getConfig() {
  if (fs.existsSync('plox.json')) {
    return JSON.parse(fs.readFileSync('plox.json'));
  } else {
    showError();
    c(chalk.red('plox config not found'));
    c(chalk.magenta(`${appName} new my-epic-plox-site-2020`), chalk.blue(` - Creates a plox site`));
...
if (args[0]) { //we need a command to run anything at all
  ...
  if (command === 'new') {
    ...
    // to run this command we need a site name
    if (!subject) {
      showError();
      c(chalk.red('No site name provided'));
      c(chalk.blue(`Try: ${appName} new my-epic-site-2020`));
    }
    ...
    if (subject && slugRegex.test(subject)) { //testing to see
      ...
      //send payload to new site directory
      fs.copySync(appRoot + '/payload', subject);

      c(chalk.green('Created site: ' + subject));

    } else {
      showError();
      c(chalk.red('site is not a valid file name. site name must-be-in-slug-format.'));
  ...
  if (command === 'page') {
    ...
    if (subject && slugRegex.test(subject)) {
      ...
      if (fs.existsSync('plox.json')){
        ...
        const fileName = appDirIn + '/' + subject + '.json';
          ...
          fs.writeFileSync(fileName, markdownContent, 'utf8', function (err) {
              ...
              c(chalk.green(`${subject}.json json created`));
          });
        }
  ...
  if (command === 'build') {
    ...
    plox.build(appDirIn, appDirOut, siteConfig);
    process.exit()
  }
  if (command === 'serve') {
    const siteConfig = getConfig();
